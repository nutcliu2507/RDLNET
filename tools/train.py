...
def main():
    ...
    all_papams = total_params(model)
    logger.info(f'all_papams: {all_papams}')
    # test_input = torch.randn(1, 15, 3, 112, 112)
...
def total_params(model):
    params = list(model.parameters())#所有參數放在params裏
    k = 0
    for i in params:
        l = 1
        for j in i.size():
            l*=j     #每層的參數存入l，這裏也可以print 每層的參數
        k = k+l   #各層參數相加
