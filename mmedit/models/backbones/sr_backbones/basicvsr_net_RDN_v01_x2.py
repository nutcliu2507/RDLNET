...
class BasicVSRNet_RDN_v01_x2(nn.Module):
    ...
    def __init__(self,
        ...
        self.backward_RDN = RDN(self.n_RDB, growRate0, growRate, nConvLayers)
        self.forward_RDN = RDN(self.n_RDB, growRate0, growRate, nConvLayers)

        self.reduction = nn.Conv2d(self.mid_channels*3, self.mid_channels, 3, 1, 1, bias=True)

        # upsample
        self.upconv1 = nn.Conv2d(self.mid_channels, self.mid_channels * 4, 3, 1, 1, bias=True)
        ...
        self.conv_last = nn.Conv2d(self.mid_channels, 3, 3, 1, 1)
    ...
    def forward(self, lqs):
        ...
        for i in range(t - 1, -1, -1):
            ...
            # feat_prop = self.lrelu(self.reduction(feat_prop))
            feat_prop = self.mish(self.reduction(feat_prop))
        ...
        for i in range(0, t):
            ...
            # out = self.last_RDN(out)
            # out, sparsity = self.SMSR_SMM(out)
            # out_sparsity.append(sparsity)
            ...
            base = F.interpolate(lqs_i, scale_factor=2, mode='bilinear', align_corners=False)
            out += base
            out_l.append(out)
