...
@masked_loss
def sparsity_loss(sparsity, current_iter, t_warm_step):
    loss_sparsity = sparsity.mean()
    current_iter_value = current_iter*t_warm_step + 0.2
    lambda_sparsity = min(current_iter_value, 1)
...
@LOSSES.register_module()
class CharbonnierSmsrLoss(nn.Module):
    """Charbonnier loss (one variant of Robust L1Loss, a differentiable
    variant of L1Loss).
    ...
    def __init__(self,
        ...
        self.t_warm_step = t_warm_step

    def forward(self, pred, target, iter, weight=None, **kwargs):
        """Forward Function.
        ...
        Args:
            ...
            weight (Tensor, optional): of shape (N, C, H, W). Element-wise
                weights. Default: None.
        """
        loss_1 = self.loss_weight * charbonnier_loss(
            pred[0],
            target,
            weight,
            eps=self.eps,
            reduction=self.reduction,
            sample_wise=self.sample_wise)
        loss_2 = self.lambda0 * sparsity_loss(
            pred[1],
            iter,
            weight,
            reduction=self.reduction,
            t_warm_step=self.t_warm_step)
