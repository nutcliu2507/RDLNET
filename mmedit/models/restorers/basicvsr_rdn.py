...
class BasicVSR_RDN(BasicRestorer):
    ...
    def __init__(self,
        ...
        self.tsa_iter = train_cfg.get('tsa_iter', 0) if train_cfg else 0
        self.with_tsa = generator.get('with_tsa', False)
        self.is_weight_fixed = False
        self.spynet_parameters = []
        for k, v in self.generator.named_parameters():
            if 'spynet' in k:
                self.spynet_parameters.append(v)

        self.TSA_parameters = []
        if self.with_tsa:
            for k, v in self.generator.named_parameters():
                if 'fusion' in k:
                    self.TSA_parameters.append(k)
    ...
    def train_step(self, data_batch, optimizer):
        ...
        # logger.info(f'TSA_parameters:{self.TSA_parameters}')
        # logger.info(f'spynet_parameters:{self.spynet_parameters}')
        # logger.info(f'all_parameters:{self.all_parameters}')
        # fix SPyNet and EDVR at the beginning

        self.generator._set_train()

        if self.step_counter < self.fix_iter:
            if not self.is_weight_fixed:
                self.is_weight_fixed = True
                for v in self.spynet_parameters:
        ...
        if self.with_tsa:
            if self.step_counter == self.tsa_iter:
                for v in self.TSA_parameters:
                    v.requires_grad_(False)
    ...
    def forward_test(self,
        ...
        # generator_copy.to('cuda')
        # generator_copy._set_test()
        # generator_copy._prepare()
        
        # logger = get_root_logger()
        # logger.info(f'flops: {flops}')

        with torch.no_grad():
            # output = generator_copy(lq)
            # flops, params = self.get_flops(generator_copy, lq)
            # logger = get_root_logger()
    ...
    def get_flops(self, net, input_imgs):
        """
        lrs (Tensor): Input LR sequence with shape (n, t, c, h, w).
        """
        img_input = torch.randn(1,15,3,64,64)
        flops, params = profile(net, (input_imgs,))
        return(flops, params)
