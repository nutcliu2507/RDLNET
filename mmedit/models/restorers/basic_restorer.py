...
class BasicRestorer(BaseModel):
    ...
    def forward_train(self, lq, gt):
        ...
        model_type =  self.generator.__class__.__name__
        # logger.info(f'model_type:{model_type}')
        loss_type = self.pixel_loss.__class__.__name__
        # logger.info(f'loss_type:{loss_type}')
        if loss_type == 'CharbonnierSmsrLoss':
            self.generator._set_tau(self.step_counter)
            loss_pix = self.pixel_loss(output, gt, self.step_counter)
            losses['loss_pix'] = loss_pix
            outputs = dict(
                losses=losses,
                num_samples=len(gt.data),
                results=dict(lq=lq.cpu(), gt=gt.cpu(), output=output[0].cpu()))
            return outputs
        else:
